...
void PC::initPC(int N, int K, float SNRdb)
{
	...
	recMessageArranged = new int[length];
	for ( int i = 0 ; i < length ; i++ )
		recMessageArranged[i] = 0;
...
void PC::noise(int * sentSymbol, float * receivedSymbol, double sqrtVariance)
{
...
void PC::noise_c(gr_complex * sentSymbol, gr_complex * receivedSymbol, double sqrtVariance)
{
	...
	// random_device rd;
	 ...
	 for (int i=0; i<length/2; i++)
		 receivedSymbol[i] = sentSymbol[i] + gr_complex(distribution(generator)/sqrt(2),distribution(generator)/sqrt(2));

}

void PC::noise_gc(gr_complex * sentSymbol, gr_complex * receivedSymbol, float sqrtVariance, int len)
{
	...
	// random_device rd;
	 ...
	 for (int i=0; i<len; i++){
		receivedSymbol[i] = sentSymbol[i] + gr_complex(n(generator)/sqrt(2),n(generator)/sqrt(2))*sqrtVariance;
		error += abs(receivedSymbol[i] - sentSymbol[i]); 
	 }
	 //cout << error << ", ";

}

void PC::noise_hgc(gr_complex * sentSymbol, gr_complex * receivedSymbol,gr_complex * hp,
					float sqrtVariance, int len)
{
	//default_random_engine generator;
	// random_device rd;
	default_random_engine gen(rand());
	normal_distribution<float> n(0,1);
	//normal_distribution<float> h(0,1);
	double pr, ps;
	gr_complex noise[len], chnl[len];
	float h_norm=0, n_norm=0, SNR=0;

	for (int i = 0; i < len; ++i)
	{
		noise[i] = gr_complex(n(gen)/sqrt(2),n(gen)/sqrt(2));
		//h_norm += pow(abs(chnl[i]),2);
		//n_norm += pow(abs(noise[i]),2);
	}
	//h_norm /= len;//h_norm = 2 if h(0,1)
	//n_norm /= len;//n_norm = 2 if n(0,1)
	//for (int i = 0; i < len; ++i){
	//	noise[i] /= sqrt(n_norm);
	//	chnl[i]  /= sqrt(h_norm);
	//}
	for (int i=0; i<len; i++){
		gr_complex nh = caching::divide_complex(noise[i],hp[i]);
		nh.real(sqrtVariance*nh.real()); nh.imag(sqrtVariance*nh.imag()); 
		receivedSymbol[i] = sentSymbol[i] + nh;
		SNR += pow(abs(receivedSymbol[i] - sentSymbol[i]),2);
		pr += pow(abs(receivedSymbol[i]),2) ;
	}
	pr /= len;
	//cout << (double) len/SNR << ", " << pr << ", " << sqrtVariance  << "],[";
}
...
void PC::computeLLR_qpsk(double * llr, gr_complex* receivedSymbol, float variance)
{
	...
	float llr1[length/2];
	float llr2[length/2];
    gr_complex s1 = gr_complex(-SQRT_TWO, -SQRT_TWO);
    gr_complex s2 = gr_complex(SQRT_TWO, -SQRT_TWO);
    gr_complex s3 = gr_complex(-SQRT_TWO, SQRT_TWO);
    gr_complex s4 = gr_complex(SQRT_TWO, SQRT_TWO);

	for (int i=0; i<length/2; i++)
	{
		 /*p0 = exp( -(receivedSymbol[i]-s1)*conj(receivedSymbol[i]-s1)/(2*variance) ) + exp( -(receivedSymbol[i]-s3)*conj(receivedSymbol[i]-s3)/(2*variance) );
		 p1 = exp( -(receivedSymbol[i]-s2)*conj(receivedSymbol[i]-s2)/(2*variance) ) + exp( -(receivedSymbol[i]-s4)*conj(receivedSymbol[i]-s4)/(2*variance) );*/
		 p0 = exp( -pow(abs(receivedSymbol[i]-s1),2)/(2*variance) ) + exp( -pow(abs(receivedSymbol[i]-s3),2)/(2*variance) );
		 p1 = exp( -pow(abs(receivedSymbol[i]-s2),2)/(2*variance) ) + exp( -pow(abs(receivedSymbol[i]-s4),2)/(2*variance) );
		 llr1[i]= log( p0/ p1);
		 p0 = exp( -pow(abs(receivedSymbol[i]-s1),2)/(2*variance) ) + exp( -pow(abs(receivedSymbol[i]-s2),2)/(2*variance) );
		 p1 = exp( -pow(abs(receivedSymbol[i]-s3),2)/(2*variance) ) + exp( -pow(abs(receivedSymbol[i]-s4),2)/(2*variance) );
		 llr2[i] = log( p0/ p1);
	}
	for (int i=0; i<length; i++){
		if(i%2 == 0){
...
void PC::SC(int * recMessage, int * recCodeword, double * recLLR, int * sentFrozen)
{
	...
	for (int i=0; i<length/2; i++)
	{
		recMessageArranged[i] = msg[loglength-2][i];
	...
	for (int i=0; i<length/2; i++)
	{
		recMessageArranged[length/2+i] = msg[loglength-2][i];
		recCodeword[2*i] = (recCodeword[2*i]+cw[loglength-2][i])%2;
		recCodeword[2*i+1] = cw[loglength-2][i];
	}

	for (int i=0; i<length; i++)
		recMessage[i] = recMessageArranged[arragedBits[i]];
